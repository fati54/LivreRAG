...
<Mermaid
  chart={`
flowchart LR
  Q["Question"] --> T["Retrieval temporel"]
  T --> R["Résultat adapté à l'année demandée"]
`}/>

Lecture : la requête est évaluée dans un contexte temporel pour récupérer la version pertinente au bon moment.
...
<Mermaid
  chart={`
flowchart TB
  C["Mémoire courte\ncache session"] --> M["Mémoire moyenne\nsession ou projet"]
  M --> L["Mémoire longue\narchive ou base durable"]
`}/>

Lecture : l’information passe de la mémoire courte vers des couches plus stables et durables.
...
<Mermaid
  chart={`
flowchart LR
  L["LLM principal"] --> R["Réponse"]
  R --> C["Critic LLM"]
  C --> V["Validation ou correction"]
`}/>

Lecture : un second modèle vérifie la réponse avant qu’elle ne soit exposée.
...
<Mermaid
  chart={`
flowchart LR
  R["Réponse"] --> G["Gouverneur de risque"]
  G --> V["Validée"]
  G --> B["Bloquée"]
  G --> E["Escaladée"]
`}/>

Lecture : le gouverneur applique les politiques et décide si la réponse est publiée, bloquée ou revue par un humain.
...
<Mermaid
  chart={`
flowchart TB
  T["Time-travel retrieval"] --> M["Mémoire pyramidale"]
  M --> G["Génération (LLM)"]
  G --> C["Critic LLM"]
  C --> R["Gouverneur de risque"]
  R --> A["Réponse antifragile"]
`}/>

Lecture : les extensions s’enchaînent pour produire une réponse plus sûre, plus contextualisée et plus fiable.
