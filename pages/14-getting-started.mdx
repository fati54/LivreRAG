...
Lecture : l’arborescence sépare clairement la configuration, les modules et l’orchestration, ce qui facilite l’évolution sans tout casser.
...
<Mermaid
  chart={`
flowchart LR
  D["Déployer"] --> M["Mesurer"] --> I["Identifier les faiblesses"] --> C["Corriger"] --> R["Redéployer"]
  R --> D
`}/>

Lecture : la boucle d’amélioration continue transforme les retours terrain en ajustements concrets.

### Bloc complémentaire : RAGOps / LLMOps

Une fois le pipeline en place, l’industrialisation passe par des pratiques “ops” :

- **Versioning des prompts** et des templates.  
- **Versioning d’index** (rebuild, rollback, double écriture).  
- **Observabilité** centralisée (traces, métriques, logs).  
- **A/B tests** sur stratégies de retrieval et prompts.  
- **Gating** des déploiements (feature flags, canary).  

**Sources**  
- [OpenTelemetry : observabilité standard](https://opentelemetry.io/docs/)  
- [MLflow : tracking et lifecycle](https://mlflow.org/docs/latest/)  
