...
<Mermaid
  chart={`
flowchart TB
  A[Docs PDF bruts] --> B[Découpage naïf en chunks]
  B --> C[Base vectorielle unique]
  C --> D[LLM (prompt concaténé)]
  D --> E[Réponse brute]
`}
/>
...
<Mermaid
  chart={`
flowchart TB
  A["Sources variées<br/>docs internes, bases légales, APIs"]
  A --> B["Ingestion robuste<br/>OCR, nettoyage, structuration, métadonnées"]
  B --> C["Indexation hybride<br/>vecteurs + BM25 + re-ranking"]
  C --> D["LLM génératif<br/>prompts contrôlés, citations obligatoires"]
  D --> E["Vérification<br/>critic LLM, risk governor, traçabilité"]
  E --> F["Réponse validée + auditable"]
`}
/>
---
