...
<Mermaid
  chart={`
flowchart TB
  I["Ingestion basique"] --> R["Retrieval vectoriel"] --> G["Génération LLM"] --> A["Réponse brute"]
`}/>

Lecture : un enchaînement minimal rapide à mettre en place, mais sans gouvernance ni vérification.
...
flowchart TB
  ...
  subgraph Generation
    G1["Templates de prompts"]
    G2["Citations obligatoires"]
    G3["Multi-LLM"]
  end
  ...
  Ingestion --> Retrieval --> Generation --> Verification --> A["Réponse validée"]
  O["Orchestrateur central"] -.-> Ingestion
  O -.-> Retrieval
  O -.-> Generation
  O -.-> Verification
`}/>

Lecture : chaque module est spécialisé, et l’orchestrateur garantit la cohérence globale et l’auditabilité.
...
<Mermaid
  chart={`
flowchart LR
  E["Exploration\nPOC rapide"] --> I["Industrialisation\npipeline modulaire"]
`}/>

Lecture : on démarre vite en exploration, puis on investit dans la robustesse pour la production.
...
- **Gartner (2024)** estime que plus de 30% des projets d’IA générative seront abandonnés après le stade du POC d’ici fin 2025, en raison de la qualité des données, de coûts mal maîtrisés et d’une gouvernance insuffisante. [Gartner press release](https://www.gartner.com/en/newsroom/press-releases/2024-06-19-gartner-says-at-least-30-percent-of-generative-ai-projects-will-be-abandoned-after-proof-of-concept-by-end-of-2025)
...
- **Google DORA Report (2025)** : 90% des développeurs utilisent désormais l’IA générative, mais l’observabilité et la gouvernance restent les principaux défis. [DORA report 2025](https://cloud.google.com/blog/products/devops-sre/dora-report-2025)
- **Stanford HAI : AI Index Report (2024)** : Les coûts d’entraînement des modèles ont explosé, renforçant l’argument économique en faveur du RAG vs le fine-tuning. [AI Index 2024 summary](https://www.weforum.org/stories/2024/05/ai-index-report-2024/)
